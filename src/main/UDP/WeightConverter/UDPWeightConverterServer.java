package main.UDP.WeightConverter;
...
public class UDPWeightConverterServer extends javax.swing.JFrame {

    private DatagramSocket serverSocket;
    private final Map<String, Double> weightUnits = new HashMap<>();
    private boolean connected = false;

    public UDPWeightConverterServer() {
        initComponents();

        weightUnits.put("kilogram", 1.0);
        weightUnits.put("gram", 0.001);
        weightUnits.put("milligram", 0.000001);
        weightUnits.put("microgram", 0.000000001);
        weightUnits.put("ton", 1000.0);
        weightUnits.put("pound", 0.453592);
        weightUnits.put("ounce", 0.0283495);
        weightUnits.put("stone", 6.35029);
    }
                ...
                try {
                    while (connected) {
                        ...
                        if ("[JOIN]".equals(msgIn)) {
                            ...
                            // Gửi danh sách các đơn vị cân nặng mà server hỗ trợ cho client
                            StringBuilder units = new StringBuilder();
                            for (String unit : weightUnits.keySet()) {
    ...
    private double convertWeight(String fromUnit, String toUnit, double value) {
        ...
        double fromFactor = weightUnits.get(fromUnit);
        double toFactor = weightUnits.get(toUnit);
